...
const CustomBreadCrumbs = React.forwardRef(function CustomBreadCrumbs(
  props,
  ref
) {
  const {
  ...
  const renderItemsBeforeAndAfter = (allItems) => {
    ...
    if (itemsBeforeCollapse + itemsAfterCollapse >= allItems.length) {
      if (process.env.NODE_ENV !== "production") {
        console.error(
          [
            "Material-UI: You have provided an invalid combination of props to the CustomBreadCrumbs.",
    ...
    .filter((child) => {
      if (process.env.NODE_ENV !== "production") {
        if (isFragment(child)) {
          console.error(
            [
              "Material-UI: The CustomBreadCrumbs component doesn't accept a Fragment as a child.",
...
export default withStyles(styles, { name: "MuiBreadcrumbs" })(
  CustomBreadCrumbs
);
